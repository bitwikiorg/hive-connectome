...
class DeterministicMiniBrain(MiniBrain):
    ...
    def step(self, payload: Any) -> NeuralObservation:
        ...
        self._feedback *= 0.5
        self.step_no += 1
        energy = sum(v * v for v in self.state) / max(1, self.size)
        novelty = abs(energy - self.prev_energy)
        self.prev_energy = energy
        metrics = {
            "mean": sum(self.state) / self.size,
            "energy": energy,
            "max_abs": max(abs(v) for v in self.state),
            "novelty": novelty,
        }
        return NeuralObservation(
            brain_id=self.brain_id,
            brain_kind=self.kind,
            engine="synthetic-deterministic-v1",
            step=self.step_no,
