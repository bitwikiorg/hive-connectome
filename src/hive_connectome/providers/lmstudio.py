...
class LMStudio:
    ...
    async def chat(self, model: str, prompt: str, context: Any, temperature: float = 0.2) -> LLMResult:
        payload = {
            ...
            "messages": [
                {"role": "system", "content": "You are an integrated reasoning member of HIVE. Process every supplied input with the neural and JEV state. Use supplied evidence only. Return ONLY JSON with keys answer (the task answer), analysis (string), confidence (number from 0.0 to 1.0 when meaningful), evidence_refs (array of strings), unresolved (array of strings), and neural_feedback (number from -1.0 to 1.0; 0.0 means deliberate no recurrent modulation)."},
