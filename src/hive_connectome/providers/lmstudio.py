...
class LMStudio:
    ...
    async def chat(self, model: str, prompt: str, context: Any, temperature: float = 0.2) -> LLMResult:
        payload={
            ...
            "messages":[
                {"role":"system","content":"You are the occasional reasoning layer inside HIVE. Use supplied evidence only. Return concise analysis and unresolved questions."},
                {"role":"user","content":f"Task:\n{prompt}\n\nState:\n{context}"}
            ],
        }
        async with httpx.AsyncClient(timeout=120, transport=self.transport) as client:
            r=await client.post(f"{self.base_url}/chat/completions", headers=self.headers, json=payload)
            r.raise_for_status()
            raw=r.json()
        text=raw.get("choices",[{}])[0].get("message",{}).get("content","")
        return LLMResult(provider="lmstudio",model=model,text=text,raw=raw)
