...
async def run(executable: str|None, screenshot: Path|None):
    ...
    async with async_playwright() as pw:
        ...
        await page.evaluate("""fx=>{window.__fx=fx;window.fetch=async(url,opts={})=>{const u=new URL(url,'http://hive.local');const p=u.pathname;let body,status=200;
          if(p==='/api/health')body={ok:true,version:'0.6.0',venice_configured:true,lmstudio_model:'tiny',neural_runtime:{backend:'cook2019_connectome -> malecns_locomotor',primary_experiment_ready:false,study_mode:'CONTROL_ONLY',control_runtime:{ready:true,backend:'cook2019_connectome -> malecns_locomotor'}}};
        ...
        assert await page.locator('#realBrainStatus').text_content()=='BLOCKED — full MaleCNS engine missing'
