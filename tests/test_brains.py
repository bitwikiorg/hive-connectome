...
def test_brain_is_stateful_and_resettable():
    b=DeterministicMiniBrain("x",BrainKind.WORM_LINK,8)
    a=b.step({"x":1}); c=b.step({"x":1})
    assert a.state_vector!=c.state_vector
    b.reset()
    assert b.step({"x":1}).state_vector==a.state_vector

def test_feedback_changes_next_state():
    a=DeterministicMiniBrain("x",BrainKind.FLY_CORE,8)
    b=DeterministicMiniBrain("x",BrainKind.FLY_CORE,8)
